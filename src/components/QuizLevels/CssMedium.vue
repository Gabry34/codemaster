<template>
  ...
  <script>
  import CssQuestions from "../QuizLevels/CssQuestions.json";
  ...
  export default {
    ...
    methods: {
      start() {
        ...
        for (const item of this.EasyQuestions.CssQuestions) {
      ...
      next() {
        if (document.getElementById("checkbox-ans1").checked || document.getElementById("checkbox-ans2").checked || document.getElementById("checkbox-ans3").checked) {
          ...
          this.router.push("/ChooseQuiz/Medium/Results");
        }
        } else {
